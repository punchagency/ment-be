...
from ..models import FileAssociation, MainData, Algo, FileColorMapping
...
def extract_and_store_colors(file_association, rows):
    """
    Scans CSV rows for hex color codes and stores unique colors
    with their semantic meaning derived from column headers.
    """
    seen = set()
    ...
    for row in rows:
        for column_name, value in row.items():
            ...
            value = value.strip()
            if HEX_COLOR_REGEX.match(value):
                color_code = value.upper()
                meaning = column_name.strip()  # ✅ stable + reliable

                seen.add((color_code, meaning))
    ...
    objs = [
        ...
        for color_code, meaning in seen
    ]

    FileColorMapping.objects.bulk_create(
        objs,
...
def fetch_ftp_bytes(file_path):
    ...
    try:
        ...
        print(f"{e} Error Occurred While Connecting")
        raise
...
def is_file_changed(file_association: FileAssociation, content_bytes: bytes) -> bool:
    ...
    changed = not (
        file_association.last_hash
        and file_association.last_hash == new_hash
    )
...
def store_csv_data(
    ...
    extract_and_store_colors(file_association, rows)

    existing_rows_by_key = {}
    existing_main_data = MainData.objects.filter(
    ...
    file_algo_name = (
        file_association.algo.algo_name
        if file_association.algo
        else "Auto-Detect"
    )
...
def parse_csv_bytes_to_dicts(csv_bytes: bytes, fa: FileAssociation, encoding="utf-8"):
    ...
    for row in reader:
        ...
        for h_original, h_clean in zip(reader.fieldnames, raw_headers):
            ...
            clean_row[h_clean] = value.strip() if value else ""
